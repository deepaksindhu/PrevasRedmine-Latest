...
import com.prevas.redmine.IssueDetailTabs.DetailsHolder;
...
public class IssueEdit extends Activity 
{
	...
	private IssuePriorityAdapter 	mPrioritiesAdapter;
	...
	private class LoadViewTask extends AsyncTask<Void, Void, Void>
	{
		...
		{	        
	        ...
	        if (null != titleTextView) {
	        	titleTextView.setText("#" + mIssue.getId().toString() + " " + mIssue.getSubject());
	        }
		}
	}
	...
	private void getDataFromIssue()
	{
		...
		try {
			...
			mAssigneeAdapter.add(PrevasRedmine.m_redmineManager.getCurrentUser().getFullName());
			...
			if (count > 0) {
				for (int i = 0; i < count; ++i) {
					User user = mMemberList.get(i).getUser();
					if (user != null) {
						//mUserList.add(user);
						mAssigneeAdapter.add(user.getFullName());
					}
				}
			}
//			else {
//				List<User> users = PrevasRedmine.m_redmineManager.getUsers();
//				count = users.size();
//				for (int i = 0; i < count; ++i)
//					mAssigneeAdapter.add(users.get(i).getFullName());
//				
//				mUserList = users;
//			}		
		} catch (Exception e) {
...
//		}
		// Priority
		try {
			List<IssuePriority> priorityList = PrevasRedmine.m_redmineManager.getIssuePriorities();
			mPrioritiesAdapter = new IssuePriorityAdapter(this, android.R.layout.simple_spinner_item, priorityList);
		} catch (RedmineException e) {
			e.printStackTrace();
		}
	}
	...
	private void loadContents()
	{
		...
		if (-1 != holder.mParentId) {
			parentIdCheckBox.setEnabled(false);
			parentIdCheckBox.setChecked(true);
			parentIdCheckBox.setText("Parent #" + Integer.toString(holder.mParentId));
		}
		else {
			parentIdCheckBox.setVisibility(0);
		}
		
//		// Parent id if it is a subtask
//		mParentIdCheckBox = (CheckBox) findViewById(R.id.check_parentId);
//		mParentIdCheckBox.setEnabled(false);
//		if (-1 != holder.mParentId) {
//			mParentIdCheckBox.setChecked(true);
//			mParentIdCheckBox.setText("Parent #" + holder.mParentId.toString());
//		} 
//		else {
//			mParentIdCheckBox.setChecked(false);
//			mParentIdCheckBox.setText("No parent");
//		}
...
//	            	}
//	            	else {
	            		String username = mAssigneeAdapter.getItem(position);
		            	User user = getUserFromMemberList(username);
...
//		            	}
        ...
        try {       	
    		...
    		spinStatus.setSelection(mPrioritiesAdapter.getItemIndexByName(holder.mPriority));    		
...
//        	    	 mIssue.setPriorityId(priorityId);
        	    	 IssuePriority priority = mPrioritiesAdapter.getItem(position);
        	    	 mIssue.setPriorityId(priority.getId());
        	    	 mIssue.setPriorityText(priority.getName());
        	     }
        	     public void onNothingSelected(AdapterView<?> adapterView) 
        	     { 
        	    	 // do nothing
    	...
    	try {
			...
			mPercentDoneSeekbar.setProgress(Integer.parseInt(holder.mPercentDone));
...
//		mIssue.setPriorityText()
	...
	{			
		User user = null;
		if (name.equals("--")) return user;
		
		try {
			user = PrevasRedmine.m_redmineManager.getCurrentUser();
		} catch (RedmineException e) {
